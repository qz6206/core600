...
NUM_WORKERS = 4  # transcript 单次 ~21k tokens，过高并发会触发 Kimi K2.5 TPM 限流
MAX_RETRIES = 3
...
要求：
1. 公司名（{name}）、人名、产品名（如 NVIDIA Corporation、Jensen Huang、Hopper、Blackwell、Quantum InfiniBand）一律保留英文原文，不要译成中文
2. 财务术语用准确的中文：
   - revenue → 营收 / 收入
   - data center → 数据中心
   - year over year (YoY) → 同比
   - quarter over quarter (QoQ) → 环比
   - fiscal Q1 / fiscal year 2026 → 2026 财年第一季度 / 2026 财年
   - hyperscale → 超大规模
   - inference / training → 推理 / 训练
   - guidance / outlook → 业绩指引 / 业绩展望
   - non-GAAP → 非 GAAP
   - gross margin → 毛利率
   - operating margin → 营业利润率
3. 数字精确（$44 billion → 440 亿美元；69% → 69%；$2.5B → 25 亿美元）
4. 保留对话格式（"操作员：" / "分析师 XXX：" / "CFO XXX：" 这种说话人标记）
5. 中文要自然，像中信证券/华泰证券研究报告或彭博中文财经新闻
6. 直接输出译文，不要加"以下是翻译"等任何说明文字
...
{content}"""
    ...
    body = {
        ...
        "max_tokens": 16000,  # transcript 译文较长
        "temperature": 0.3,
    }
    for attempt in range(MAX_RETRIES):
        try:
